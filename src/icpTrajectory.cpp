...
public:
  ...
  int hectorEncoder(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Hector, double *&M, int i, int dim, Eigen::Vector3d &mass_center){
    ...
    while(hactor_offset < Hactor_Path.poses.size() ){
      ...
      myfileH << Hactor_Path.poses[hactor_offset].header.stamp << ","  << cloud_Hector->points[i].x << "," << cloud_Hector->points[i].y << "," << cloud_Hector->points[i].z << std::endl;
      // if (factor>1){
  ...
  int kittiEncoder(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Tracker, double *&T, int j, int dim){
    ...
    while(laser_offset < Laser_Path.poses.size() ){
      ...
      myfileJ << Laser_Path.poses[laser_offset].header.stamp << "," <<  T[j*dim+0] << "," << T[j*dim+1] << "," << T[j*dim+2] << std::endl;
      // if (factor>1){
  ...
  void ITMEncoder(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_ICP, double *&M, int i, int dim){
    ...
    for (int c = 0; c < i; ++c) {
      ...
      myfileI << Hactor_Path.poses[hactor_offset-i+c].header.stamp << "," <<cloud_ICP->points[c].x << "," << cloud_ICP->points[c].y << "," << cloud_ICP->points[c].z << std::endl;
    }
  }
